...
@section('details')
	...
	<form class="form-horizontal" method="post" action="" autocomplete="off" role="form">
			...
			<div class="col-md-3">
				<select class="form-control" name="claimstype_id" id="claimstype_id">
					...
					@foreach ($claimstype as $claimtype)
						<option value="{{ $claimtype->id }}"{!! (old('claimstype_id') == $claimtype->id ? ' selected="selected"' : '') !!}>{{ $claimtype->name }}</option>
					@endforeach
				</select>
			</div>
		</div>
			...
			<div class="col-md-3">
				<select class="form-control" name="document_id" id="document_id">
					...
					@foreach ($documents as $document)
						<option value="{{ $document->id }}"{!! (old('document_id') == $document->id ? ' selected="selected"' : '') !!}>{{ $document->name }}</option>
					@endforeach
				</select>
			</div>
		</div>
			...
			<div class="col-md-1">
				<select class="form-control" name="currency_id" id="currency_id">
					...
					@foreach ($currencies as $currency)
						<option value="{{ $currency->id }}"{!! (old('currency_id') == $currency->id ? ' selected="selected"' : '') !!}>{{ $currency->name }}</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		<div class="form-group {{ $errors->has('local') ? 'has-error' : '' }}">
			...
			<div class="col-md-3">
				<p class="form-control-static" id="local">0.000</p>
			</div>
		</div>
