...
@section('content')
	...
	<div class="page-header"><h3><span class="glyphicon glyphicon-user"></span> Edit</h3></div>
	...
	<form class="form-horizontal" method="post" action="" autocomplete="off" role="form">
		...
		<div class="form-group {{ $errors->has('department') ? 'has-error' : '' }}">
			...
			<div class="col-md-10">
				<input type="text" class="form-control" name="department" id="department" value="{{ old('department', $result->department) }}" />
			</div>
		</div>
			...
			<div class="col-md-10">
				<select class="form-control" name="user_id" id="user_id">
					...
					@foreach ($users as $user)
						<option value="{{ $user->id }}" {{ ( $user->id == old('user_id', $result->user_id) ? ' selected="selected"' : '') }}>{{ $user->display_name }}</option>
					@endforeach
				</select>
			</div>
		</div>
			...
			<div class="col-md-10">
				<select class="form-control" name="site_id" id="site_id">
					...
					@foreach ($sites as $site)
						<option value="{{ $site->id }}" {{ ( $site->id == old('site_id', $result->site_id) ? ' selected="selected"' : '') }}>{{ $site->name }}</option>
					@endforeach
				</select>
			</div>
		</div>
