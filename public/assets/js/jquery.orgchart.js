...
(function($) {
    ...
    $.fn.orgChart = function(options) {
        ...
        return this.each(function() {
            ...
            if ($root) {
                buildNode($root, $container, 0, 0, opts, "L0N0");
    ...
    $.fn.orgChart.defaults = {
		...
		targetLevel: -1,
		targetNode : -1,
		ParentNode : "level0-node0"
    };

    function buildNode($node, $appendTo, level, index, opts, parent) {
        ...
        if ($childNodes.length > 0) {
            if (opts.depth == -1 || level+1 < opts.depth) {
                ...
                $childNodes.each(function(index) {
                    var $td = $("<td/>");
                    $td.attr("colspan", 2);
                    buildNode($(this), $td, level+1, index, opts, parent+"-L"+(level+1)+"N"+index);
...
/*
		...
		if (index != opts.targetNode) {
			$nodeRow.nextAll("tr").hide();
		}

		if (level != opts.targetLevel && index == opts.targetNode) {
			$nodeRow.nextAll("tr").hide();
		}

		if ($nodeRow[0].id == "L0N0-L1N0-L2N0") {
			$nodeRow.nextAll("tr").show();
		}
*/
		if (index == 0 && level == 0) {
			$nodeRow.nextAll("tr").show();
